...
namespace rrt_planner {
    ...
    void RRTPlannerROS::initialize(std::string name, 
        ...
        if(!initialized_) {
            ...
            //laser_subscriber = nh.subscribe<sensor_msgs::LaserScan>("/scan", 1, &RRTPlannerROS::laserCallback, this);
...
/*
    void RRTPlannerROS::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan) {
    ...
    bool RRTPlannerROS::isPathClear() {
    // Use the updated obstacle information from laser scan data to check if the path is clear
    // Return true if the path is clear, false otherwise

    // Access the most recent laser scan data in your laserCallback function
    // You should have a member variable to store the latest laser scan data
    // For example: sensor_msgs::LaserScan latest_scan_data;

    // Check the laser scan data for obstacles
    // You can iterate through the ranges and check if any range value is less than a certain threshold
    // If a range is below the threshold, it indicates an obstacle
    // Adjust the threshold to suit your specific obstacle detection requirements
    
    double obstacle_threshold = 0.3;
    for (int i = 0; i < latest_scan_data.ranges.size(); i++) {
        if (latest_scan_data.ranges[i] < obstacle_threshold) {
            
            return false;
        }
    }

    // If no obstacles are detected, the path is clear
    return true;
}*/
